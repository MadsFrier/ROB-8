...
def lseg_image(img_save_dir, cs=0.05, gs=1000):
    mask_version = 1 # 0, 1

    crop_size = 480 # 480
    base_size = 520 # 520
    lang = "person, conveyor belt, hat, floor, package,other"
    labels = lang.split(",")
    ...
    img_path = '/workspaces/ROB-8/docker/src/content/test_data/test3.jpg'
    crop_size = 480
    padding = [0.0] * 3
    ...
    image = transform(image1).unsqueeze(0)
    ...
    with torch.no_grad():
        #outputs = evaluator.forward(image, labels) #evaluator.forward(image, labels) #parallel_forward
        #outputs = model(image,labels)
        outputs, pred = get_lseg_feat(model, image1, labels, transform)
